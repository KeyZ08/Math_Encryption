...
public static class Galois
{
    ...
    /// Извлекает целое значение из поля Галуа, соответствующее заданному показателю степени.
    public static int GetIntValFromAlphaExp(int exp)
        => GaloisFieldByExponentAlpha[exp];

    /// Извлекает показатель степени из поля Галуа, соответствующий заданному целочисленному значению.
    public static int GetAlphaExpFromIntVal(int intVal)
    {
        if (intVal == 0)
            throw new ArgumentOutOfRangeException(nameof(intVal), "Ноль находится за пределами допустимого диапазона.");
        return GaloisFieldByIntegerValue[intVal];
    }

    /// Нормализует показатель поля Галуа, чтобы он оставался в пределах размера поля.
